...
# Check if running on Arch or BlackArch
check_arch() {
    if ! grep -Eq "^(NAME=\"Arch Linux\"|NAME=\"BlackArch Linux\")" /etc/os-release; then
        print_status "System Check" "FAIL"
        echo -e "${RED}This script is designed for Arch Linux and BlackArch Linux only${NC}"
        exit 1
    fi
    print_status "System Check" "OK"
}
...
disable_metered() {
    print_section "Configuring Network Connection"
    
    local connections=$(nmcli -t -f NAME,DEVICE,STATE c show --active | grep activated)
    if [ -z "$connections" ]; then
        print_status "Network Connection Status" "FAIL"
        echo -e "${RED}No active connections found${NC}"
        return 1
    }

    while IFS=':' read -r name device state; do
        echo -e "${DIM}Configuring connection: $name${NC}"
        nmcli connection modify "$name" connection.metered no
        print_status "Disabled metered connection for $name" "OK"
    done <<< "$connections"
}
...
test_configuration() {
    ...
    local test_domains=("google.com" "cloudflare.com" "github.com" "archlinux.org")
    local total_time=0
...
main() {
    ...
    if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
        ...
        echo "  This tool optimizes your DNS configuration for Arch Linux"
        echo "  systems for better performance and security. It automatically"
        echo "  selects the fastest DNS servers and configures the system"
        echo "  appropriately."
        echo
        exit 0
    fi
    ...
    print_header() {
        ...
        echo
        echo -e "${DIM}Metered connection has been disabled for all active connections${NC}"
        echo
    }
